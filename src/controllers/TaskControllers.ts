...
// O req.body espera estes objetos {title, description, status, priority}
...
export class TaskController {
    ...
    delete: Handler = (req, res) => {
        const { id } = req.params
        const deltedTask = Task.delete(+id)
        if(!deltedTask) throw new HttpError(404, "task not found")
        res.json(deltedTask)
    }
}